...
const { transcodeBE, transcodeLE } = require('./codec/transcode')
...
for (let endian of ['BE', 'LE']) {
  ...
  for (let type of INT_TYPES) {
    ...
    let TypedArray = GLOBAL[viewType + 'Array']
    ...
    if (size === 8) {
      ...
      Buffer.prototype['read' + fullType] = function (offset = 0) {
        ...
        let target = new TypedArray(1)
        transcode(8, size * 8, this, target, 0, offset, offset + size)
      ...
      Buffer.prototype['write' + fullType] = function (value, offset = 0) {
        ...
        let source = new TypedArray([value])
        transcode(size * 8, 8, source, this, offset, 0, 1)
