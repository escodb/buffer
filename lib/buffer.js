...
for (let endian of ['BE', 'LE']) {
  ...
  for (let type of INT_TYPES) {
    ...
    for (let bitSize of INT_SIZES) {
      let fullType = type + bitSize + (bitSize === 8 ? '' : endian)
    ...
    let get64 = 'getBig' + type.replace(/UInt/, 'Uint') + '64'
    let set64 = 'setBig' + type.replace(/UInt/, 'Uint') + '64'

    Buffer.prototype['readBig' + type + '64' + endian] = function (offset = 0) {
      checkRange(offset, 'offset', 0, this.length - 8)

      let view = getView(this.buffer)
      return view[get64](offset, isLE)
    }

    Buffer.prototype['writeBig' + type + '64' + endian] = function (value, offset = 0) {
      checkRange(offset, 'offset', 0, this.length - 8)

      let view = getView(this.buffer)
      view[set64](offset, value, isLE)
      return offset + 8
    }
  }
}

for (let method in Buffer.prototype) {
  let alias = method.replace(/UInt/, 'Uint')
  Buffer.prototype[alias] = Buffer.prototype[method]
}
