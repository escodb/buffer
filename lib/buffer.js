...
for (let endian of ['BE', 'LE']) {
  ...
  for (let type of INT_TYPES) {
    for (let bitSize of INT_SIZES) {
      ...
      arrays.set(viewType, new GLOBAL[viewType + 'Array'](1))
      ...
      if (bitSize === 64) {
        ...
        Buffer.prototype['read' + fullType] = function (offset = 0) {
          ...
          let target = arrays.get(viewType)
          transcode(8, bitSize, this, target, 0, offset, offset + byteSize)
        ...
        Buffer.prototype['write' + fullType] = function (value, offset = 0) {
          ...
          let source = arrays.get(viewType)
          source[0] = value
          return transcode(bitSize, 8, source, this, offset, 0, 1)
        }
      }
